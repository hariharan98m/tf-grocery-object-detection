...
import pdb
from fsdet.data.meta_grocery import grocery_dataset_loader
def register_grocery():
    for s in ['all', 'novel']:
        metadata = _get_builtin_metadata("grocery")
        # pdb.set_trace()
        # register dataset (step 1)
        name = 'grocery_'+s
        DatasetCatalog.register(
            name, # name of dataset, this will be used in the config file
            lambda: grocery_dataset_loader( # this calls your dataset loader to get the data
                name, metadata["thing_classes"], # inputs to your dataset loader
            ),
        )
        ...
        MetadataCatalog.get(name).evaluator_type = "grocery_dataset" # set evaluator
